...
func Build() error {
	...
	if err := sh.RunWith(alfredEnv(), "go", "build", "-o", "./build/alsf", "."); err != nil {
		return err
	}
...
func Run() error {
	...
	return sh.RunWith(alfredEnv(), "./alsf", "-h")
}
...
func Config() {
	...
	fmt.Println("Alfred prefs file:", PrefsFile)
	fmt.Println("Alfred sync folder:", SyncFolder)
	fmt.Println("Alfred workflow directory:", WorkflowDir)
	fmt.Println("Workflow data directory:", DataDir)
	fmt.Println("Workflow cache directory:", CacheDir)
}
